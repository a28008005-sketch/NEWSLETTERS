...
[고래영어 학생관리](https://app.notion.com/p/2bee4c5088208117b9c6df9593f4fe96) 맨 위의
**워크시트 만들기 · 생성 요청** 에 한 줄 적고 상태를 `대기` 로 두면 됩니다.
...
| --- | --- |
| 아직 만들지 않은 기사 고르기 | 자동 |
| 본문·소제목·사진·음원 주소 받기 | 자동 |
| 단어와 문항, 쓰기 과제 쓰기 | Claude 에게 "요청 처리해줘" |
| PDF 3종 만들기 | 자동 |
| 마스터 목록에 첨부하고 요청을 완료로 바꾸기 | 자동 |

문항을 쓰는 단계만 판단이 필요해서 사람 손을 거칩니다. 나머지는 전부 규칙으로 처리합니다.
발행이 끝나면 요청 행이 저절로 `완료` 로 바뀌고 결과 칸에 링크가 적힙니다.

기사 고르기는 저장소의 **Actions → 요청받은 기사 고르기 → Run workflow** 로도 돌릴 수 있습니다.
