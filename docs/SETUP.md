...
대시보드를 쓰려면 두 가지가 더 필요합니다.

**첫째, 통합을 요청 데이터베이스에도 연결합니다.**
[영자신문 워크시트 대시보드](https://app.notion.com/p/3d9e4c508820812cbbafebecf70eff9e) 를 열고
오른쪽 위 ··· → 연결 → 연결 추가에서 `워크시트 자동화` 를 고릅니다.
2단계에서 마스터 목록에 한 것과 같습니다.

**둘째, Claude API 키를 등록합니다.**
문항을 만드는 일만 사람의 판단이 필요해서 Claude 에게 맡깁니다.

1. https://console.anthropic.com 에서 API 키를 만듭니다.
2. [Secrets 설정](https://github.com/a28008005-sketch/NEWSLETTERS/settings/secrets/actions) 에서
   이름 `ANTHROPIC_API_KEY` 로 등록합니다.

키가 없으면 기사 본문까지만 받아 두고 요청은 `실패` 로 표시됩니다.
워크시트 한 건을 만드는 데 드는 비용은 크지 않지만, 사용한 만큼 청구됩니다.
...
| 요청이 계속 `대기` 상태 | 통합을 요청 데이터베이스에 연결하지 않았습니다. 5단계를 확인하세요. |
| 요청이 `실패` 로 바뀜 | 결과 칸에 이유가 적혀 있습니다. 대부분 레벨을 못 읽었거나 API 키가 없는 경우입니다. |
